...
// const cities = data.forEach(data => {
...
// const coordsArray = [];
// data.forEach(data => {
//   let latlng = data.address.geo.lat + ", " + data.address.geo.lng;
...
let arrayofStrings = [];
data.forEach(data => {
  let catchPhraseAndBs = `catchPhrase: ${data.company.catchPhrase}, BS: ${data.company.bs}`;
  arrayofStrings.push(catchPhraseAndBs);
});

console.log(arrayofStrings);

// query the HTML for the class class-week, then use forEach to change the text content
// * used live server to open week 4 class activity to view DOM Practice in web browser.
